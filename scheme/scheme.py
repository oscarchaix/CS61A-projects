...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    elif expressions.rest is nil: # PB 10 -- TAIL RECURSION
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol not in self.bindings.keys() and self.parent is None:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        f, params, body = target.first, target.rest, expressions.rest
        env.define(f,LambdaProcedure(params,body,env))
        return f
        # END PROBLEM 9
    else:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        # FROM PROBLEM 9
        f, params, body = target.first, target.rest, expressions.rest
        env.define(f,MacroProcedure(params,body,env))
        return f
        # END FROM PROBLEM 9
    else:
        bad_target = target.first if isinstance(target, Pair) else target
