...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.is_scared is False and self.direction == -1:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        if curr_place.bees and self.min_range == 0:
            return random_or_none(curr_place.bees)
        else:
            while curr_place.entrance is not beehive:
                curr_place = curr_place.entrance
                distance += 1
                if curr_place.bees and distance >= self.min_range and distance <= self.max_range:
                    return random_or_none(curr_place.bees)
        # END Problem 3 and 4
...
class BodyguardAnt(Ant):
    ...
    name = 'Bodyguard'
    food_cost = 4
    is_container = True # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 9
    implemented = True   # Change to True to view in the GUI
    # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    name = 'Tank'
    damage = 1
    food_cost = 6 # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 10
    implemented = True   # Change to True to view in the GUI
    # END Problem 10
...
class ScubaThrower(ThrowerAnt):
    '''Is watersafe, so its armor won't be reduced to 0 when it is placed in a Water Place'''
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem 13
    ...
    true_queen = 0
    implemented = True   # Change to True to view in the GUI
    all_buffed = [] ###Need to change in class to keep changes
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        def true_queen():
            if QueenAnt.true_queen == 0:
                self.true_queen = True
                QueenAnt.true_queen += 1
            else:
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            queen_place = self.place
            while queen_place.exit is not None:
                queen_place = queen_place.exit
                if queen_place.ant:
                    if queen_place.ant not in QueenAnt.all_buffed:
                        queen_place.ant.damage *= 2
                        QueenAnt.all_buffed.append(queen_place.ant)
                if isinstance(queen_place.ant,BodyguardAnt) and queen_place.ant.contained_ant and queen_place.ant.contained_ant not in QueenAnt.all_buffed:
                    queen_place.ant.contained_ant.damage *=2
                    QueenAnt.all_buffed.append(queen_place.ant.contained_ant)
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    before = bee.action
    after = effect(bee.action,bee)

    def differentiatied_action(colony):
        nonlocal duration
        if duration > 0:
            after(colony)
        else:
            if effect.__name__ == 'make_scare':
                bee.is_scared = True
                bee.direction = -1
            before(colony)
        duration -= 1

    bee.action = differentiatied_action
    # END Problem EC
